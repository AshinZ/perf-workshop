...
static inline int gen_alloc_enter(struct pt_regs *ctx, size_t size) {
        ...
        if (SHOULD_PRINT)
                bpf_trace_printk("alloc entered, size = %u, tid = %u\\n", size,  tid);
        return 0;
}
...
static inline int gen_alloc_exit2(struct pt_regs *ctx, u64 address) {
        ...
        if (SHOULD_PRINT) {
                bpf_trace_printk("alloc exited, size = %lu, result = %lx, tid = %u\\n",
                                 info.size, address, tid);
        }
        return 0;
}
